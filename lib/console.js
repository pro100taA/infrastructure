const { node: { readline, util, path } } = require('./dependencies.js');
const application = require('./application.js');

const regexp = new RegExp(path.dirname(application.path), 'g');
const shortenStack = msg => msg.replace(regexp, '');
...
class Console {
    ...
    trace(...args) {
        ...
        const stack = shortenStack(err.stack).slice(5);
