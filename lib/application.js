...
class Application extends events.EventEmitter {
    constructor() {
        ...
        this.staticPath = path.join(this.pathRoot, 'web');
    ...
    async init() {
        ...
        await Promise.allSettled([
            ...
            (async () => {
                ...
                await this.loadDomain(this.domainPath);
            })(),
    ...
    createSandbox() {
        ...
        const sandbox = {
            ...
            application, node, npm,
            api, lib, domain, config,
    ...
    async loadModule(fileName) {
        ...
        for (let depth = 0; depth <= last; depth++) {
            ...
            if (next) {
                ...
                level[namespace] = iface.handle || iface;
                container.parent = level;

                if (depth === MODULE && namespace === 'start') {
                    this.starts.push(iface.handle);
                }
            }
    ...
    async loadDomain(placePath) {
        ...
        const { domain, console, lib, application:{ db } } = this.sandbox;
        ...
        for (const item of domains) {
            ...
            domain[item.name] = new Domain(filePath, { domain, console, lib, db });
        }
    }
